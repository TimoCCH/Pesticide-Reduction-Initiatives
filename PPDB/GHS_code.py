...
input_file = 'GHS_list.csv'
output_dir = '../PPDB'  # Directory to save the output CSV
output_file_name = 'GHS_result.csv'  # Output file name
...
def main_scraping_process():
    ...
    # Load the CSV to get the list of products to search for
    ghs_path = os.path.join(output_dir, input_file)
    ghs_df = pd.read_csv(ghs_path)
    products_to_search = ghs_df['product'].tolist()
    print(f"Loaded {len(products_to_search)} products to search for from GHS.csv")
    ...
    for name, url in first_page_links.items():
        if any(product.lower() in name.lower() for product in products_to_search):
